...
select (select count(*) from employees where gender = 'M') as '남자', (select count(*) from employees where gender = 'F') as '여자';

-- 문제4.
-- 현재 근무하고 있는 직원 수는 몇 명입니까? (salaries 테이블을 사용합니다.) 
select count(distinct(emp_no)) 직원수 from salaries;
