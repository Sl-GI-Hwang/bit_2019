package kr.co.itcen.network.echo2;
...
public class RequestHandler extends Thread {
   ...
   static {
	   documents = RequestHandler.class.getClass().getResource("/webapp").getPath();
   }
...
//         outputStream.write( "<h1>이 페이지가 잘 보이면 실습과제 SimpleHttpServer를 시작할 준비가 된 것입니다.</h1>".getBytes( "UTF-8" ) );
   ...
   private void responseStaticResource(OutputStream outputStream, String url, String protocol) throws IOException {
      ...
      File file = new File(documents + url);
      if(!file.exists()) {                           //존재하지 않는 파일을 요청하면
    	  consoleLog("FIle Not Found :" + url);
    	  //response400error(outputStream, tokens[2]);      //404 error를 반환
      }
